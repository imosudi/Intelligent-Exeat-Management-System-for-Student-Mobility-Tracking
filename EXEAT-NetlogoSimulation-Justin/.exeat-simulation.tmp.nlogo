turtles-own [ role state request-time approved? frequency parent-response-time ]
...
to setup
  ...
  create-turtles 50 [
    ...
    set state "idle"
    set approved? false
    set frequency random 5
    set request-time 0
    set parent-response-time 0
    set color blue
  ]

  ;; Label and color key patches to represent other roles
  ...
  ask patches with [pxcor = 0 and pycor = min-pycor] [
    set plabel "Security"
    set plabel-color black
    set pcolor gray
  ]
...
to go
  ask turtles [
    if role = "student" [
      ...
      if state = "idle" and random 100 < 5 [
        set state "requesting"
        set request-time ticks
      ]

      ;; Request in process
      if state = "requesting" [
        set state "waiting-approval"
      ]

      ;; Parent/admin approval stage
      if state = "waiting-approval" [
        if frequency > 3 [
          ;; simulate denial
          set state "idle"
        ]
        if frequency <= 3 and ticks - request-time > 3 [
          set parent-response-time random 5
          if parent-response-time < 4 [
            set approved? true
            set state "approved"
          ]
        ]
